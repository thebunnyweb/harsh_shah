...
const CharacterFormLazy = React.lazy(()=>(import("../../components/characterform")))
...
class AddCharacterContainer extends React.Component {
  ...
  validateForm = (key=null, returnError=false) => {
    ...
    if(key){
      let validationConditions = validationRules.filter(item=>item.input === key);
      if(validationConditions.length > 0){
        validationConditions[0]['condition'].forEach((item)=>{
          ...
          if(flag === 'fail'){
            errors[key] =  Validation.errorMessages[key]['required'] || "Field Error"
          }else if(flag === "pass"){
  ...
  formSubmitValidation = (e)=> {
    ...
    if(Object.keys(errors).length > 0){
      ...
      this.setState({
        ...
        addCharacter(this.state.formData).then(()=>{
          this.setState({
            ...this.state,
            triggerRefFocus: null,
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        {postSuccess && ( <Alert data={{message:"Character Added Successfully"}} flag="success"/>)}
        {speciesDataError ? <Alert data={speciesDataError} flag="error"/> : (<Suspense fallback={<div>Loading Form...</div>}>
          <CharacterFormLazy state={this.state} submitForm={this.formSubmitValidation} handleChange={this.handleChangeState} />
        </Suspense>)}
